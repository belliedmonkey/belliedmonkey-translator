...
board('E-NoReach.dc.html', 2040, 940, '被数据否掉的（二）：任何延时提醒都触达不到', `
${head('我上一版说「复核那条直接做，有硬证据」—— 错了', '用户问「有没有人点完以后没卡、但 7 天后还回到 App」。答案：一台都没有')}

<div>
  <span class="tag-now">原来的提议（已作废）</span>
  <div class="tl" style="margin-top:8px">
    ${step('第 0 天', banTap(btn('我已打开', 's')), '18 台点过「我已打开」且从来没有过卡。')}
    <span class="arr">→</span>
    ${step('点完当下', homeOnly(), '横幅收起。')}
    <span class="arr">→</span>
    ${step('第 1 天', `<div class="ph-sm"><div class="err">${ic('alert')}<div><b>0 台</b>隔天回来过</div></div></div>`, '不是「少」，是零。')}
    <span class="arr">→</span>
    ${step('第 7 天 · 这块横幅', `<div class="ph-sm"><div class="err">${ic('alert')}<div><b>0 台</b>能看到它<br><span class="code">其中 7 台已经过了 7 天窗口</span></div></div></div>`,
      '画得再对也没用 —— 没有人会在那一天打开 App。')}
  </div>
</div>

${grid(2, `
  ${cell('更狠的一条', '不只是「隔天不回来」',
    `<div class="num">${stat('18', '点过「我已打开」且没卡')}${stat('16', '点完之后<b>再没有任何事件</b>', 'bad')}${stat('0', '点完半小时后还在', 'bad')}</div>
     ${hint('<b>「我已打开」基本是这个人在这个 App 里的最后一个动作。</b>任何在它<b>之后</b>触发的东西 —— 7 天后、1 天后、哪怕半小时后 —— 都到不了人。')}`)}
  ${cell('还活着的窗口只有一个', 'App 会话的形状',
    `<div class="num">${stat('176 / 247', '整个生命周期不到 5 分钟', 'bad')}${stat('66', '有过 3 分钟以上的「离开再回来」', 'good')}${stat('33', '其中超过半小时', 'good')}</div>
     ${hint('247 台里 223 台首末事件在同一天，跨度中位数是 <b>0 秒</b>。但有 <b>66 台</b>出现过「隔几分钟又回来」—— 那正是<b>去 Safari 弄一下再回来</b>的形状。<b>这是唯一还活着的窗口。</b>')}`)}
`)}
${ask('所以能设计的只剩两处', '① <b>第一次那几分钟里说什么</b> —— 没有第二次机会，这一次说不动就结束了。<br>② <b>「去 Safari 再回来」那一段怎么闭环</b> —— 66 台走过这条路，而我们对它<b>一个数都没有</b>（检测页那一侧全黑）。<br><br>至于真机上「误点了没法撤销」那个坑：改用<b>设置里一个重置入口</b>（一行）解决，不是靠 7 天后自动回来 —— 那条路已经证明触达为 0。')}
`, { page: 'p1' });
...
idx.notes.t1 = { x: 0, y: -320, text: '数据说了什么 —— 两条被否掉、一条要先补、三条只是判断', kind: 'title1', maxW: 3600, page: 'p1' };
...
idx.notes.ask = { x: NX, y: 0, w: 540, maxH: 560, page: 'p1', color: 'blue',
  text: '2026-09-21 复盘改过一版：第一版把「教程 → 状态行」当主改动，理由是「104 台只看过一次」。按「点了什么」分组算完，那个理由不成立 —— 点过主按钮的人并不比谁都没点的人更可能收到卡。\n\n这一页按证据强弱重排，顺序就是建议做的顺序：\n\n① 先补四条埋点 —— 检测页那一侧最值钱，因为那是唯一还活着的路\n② 我原来说「复核那条直接做」也被否了：18 台点过「我已打开」且没卡的人里，16 台点完再没有任何事件，隔天回来 0 台。任何延时提醒触达为 0\n③ 剩下三条是判断，可以现在拍板，但要分开出货\n\n两次被数据打回之后，还站得住的只有一句：**没有第二次机会，一切都得在第一次那几分钟里闭环。**' };
