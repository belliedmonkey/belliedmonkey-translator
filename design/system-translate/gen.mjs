...
const NX = ROW_W + 160;
sticky('why', 'p1', NX, 0, '怎么读这张画布：每块板 = 一个状态或一个分支。\n\n2026-09-19 两个尖刺（T1 = iPhone 真机、T2 = Mac 沙盒）都量完了：两个面都没有阻断项。板上写着「T1 读数 / T2 读数」的句子 = 真机上量出来的事实，原先「两支都画」的板已收敛成一支。仍带蓝色小标的三处（iPad 形态、长文本展开、个别 App 的服务菜单）= 还没量到，不挡路。', { color: 'orange', maxH: 360 });
sticky('t12', 'p1', NX, 800, '尖刺带回来、改了板的七处：\n① iPhone：第一次用时系统自己先弹一页告知，我们那一行放在它后面、不可点。\n② iPhone：弹层里能把 App 拉起来 ⇒ 「拉不起 App」那一支删掉，换成「点了之后落在哪」。\n③ iPhone：国行机上「默认翻译 App」可选 ⇒ 中国版照常带。\n④ Mac：增强取词授权后要重开 App 才生效。\n⑤ Mac：系统收紧剪贴板后，后台读取会卡住 ⇒ 1 秒超时，当作没取到。\n⑥ Mac：一台机器上第一次文字识别要 20 秒量级 ⇒ 多一行说明 + 空闲预热。\n⑦ Mac：从右键「服务」进来系统会把我们拉到前台 ⇒ 立刻把焦点还回去。', { color: 'purple', maxH: 420 });
