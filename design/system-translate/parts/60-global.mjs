...
${tl('iPhone · 第一次用系统翻译', '弹层底部一行：选中的文字会直接发给你配置的引擎，不经过我们的服务器。', '（无）', '—')}
${tl('Mac · 装上 / 升级后', '首页介绍卡：三种用法。不要任何权限。', '（无）', '—')}
${tl('Mac · 第一次关主窗口', '「大肚猴还在菜单栏」一次性提示。', '（无）', '—')}
${tl('Mac · 打开「增强取词」', '说明卡：会做什么 / 不会做什么。', '「想要控制这台电脑」', '开关弹回，零权限路径照常')}
...
${tl(`Mac · 较新系统读剪贴板 ${pend('T2')}`, '设置块里提前说：可能会再问一次「允许粘贴」。', '「想要粘贴」', '这一次不翻；不反复弹')}
...
${cell(`中国区设备上「默认翻译 App」这一行出不出现 ${pend('T1')}`, '', `<div class="note">若不出现：中国版 App 不带系统翻译这个部件，设置块与首页发现卡整个不出现 —— 不留一个点了没用的入口。Mac 的快速翻译不受影响。</div>`)}
