...
${cell('③ 译文', '语言对由我们自己判（系统不给源语言）；目标语言可点开改，只在这一次弹层内生效。', phone(sheet({ body: trbox(S.zh, copyRow), foot: '' })))}
`)}`, { page: PG });

// 4 · 可替换
board('SheetReplace.dc.html', 1200, 900, '可替换原文', `${head('原文可替换时（系统给 allowsReplacement = true）', '只在可编辑的地方出现：备忘录、邮件正文、输入框。')}
${grid(3, `
${cell('可替换', '一个主按钮「替换原文」。<b>没有「完成」</b> —— 关闭走弹层右上角系统那个 ✕。', phone(sheet({ body: trbox(S.zh, copyRow), foot: btn('替换原文', 'p') })))}
${cell('不可替换', '「替换原文」<b>不存在</b>，不是灰掉 —— 灰按钮会让人以为是自己没弄对。此时底下只剩「复制译文」。', phone(sheet({ body: trbox(S.zh, copyRow), foot: '' })))}
...
${grid(3, `
${cell('原文已经是目标语言', '中文用户选了一段中文 ⇒ 自动反向译成英文，语言 chip 变绿并写明原因；不出「无需翻译」这种死路。', phone(sheet({ src: S.zhSrc, lang: ['简体中文', 'English'], langOpt: { auto: true, note: '原文已是中文' }, body: trbox(S.enFromZh, copyRow), foot: '' }), { sel: S.zhSrc }))}
${cell('点开目标语言', '只影响这一次弹层 —— 扩展回写不了 App 的设置。菜单最后一行指路。', `<div style="position:relative">${phone(sheet({ body: trbox(S.zh), foot: '' }))}${langMenu}</div>`)}
...
${cell('到了超时上限', '用 App 里设的「单次超时」；文案与网页翻译同一句。', phone(sheet({ engine: '免费额度', body: err('端点没有在超时前回应。', 'timeout'), foot: btn('重试', 'p') })))}
`)}${mini('取消 = 真的中断请求（不是只关界面）；已经花掉的额度照实计。')}`, { page: PG });

// 8 · 失败分态
const E = [
  ['断网', '现在没有网络。连上之后再试。', 'network', ['重试']],
  ['超时', '端点没有在超时前回应。', 'timeout', ['重试']],
  ['key 被拒', '服务商拒绝了这把 key（HTTP 401/403）。检查 key 是否填对、有没有这个模型的权限。', 'auth', ['打开大肚猴翻译']],
  ['额度用完（你的）', '免费额度已经用完了。你可以填一把自己的 key 继续用。', 'credit_exhausted', ['打开大肚猴翻译']],
  ['额度池空了（我们的）', '不是你用完了 —— 是我们这边的免费额度池空了，正在补。先用自己的 key，或者稍后再来。', 'grant_unavailable', ['重试']],
  ['模型被改过', '免费额度只能用它指定的那个模型。你在「详细」里改过模型 —— 改回去，或者填一把自己的 key。', 'model_not_allowed', ['打开大肚猴翻译']],
  ['其它 HTTP', 'HTTP 503 —— 服务端拒绝了这次请求。', 'http · 503', ['重试']],
  ['引擎不认识', '这个版本不认识当前存着的引擎，请在 App 里重新选一个。', 'unknown_provider', ['打开大肚猴翻译']],
  ['思考耗尽', '模型把整个输出预算用在了思考上，没有产出译文。到设置的「高级参数」里调高「单次最大输出长度」，或换一个非推理模型。', 'reasoning_starved', ['打开大肚猴翻译']],
];
const mini_sheet = (t, c, b) => `<div class="sheet" style="margin-top:0; border-radius:22px; box-shadow:0 2px 8px rgba(0,0,0,.08)"><div class="sheet-h"><span class="brand"><span class="mark"></span>大肚猴翻译</span></div><div class="src q" style="font-size:.84rem">${S.en}</div>${err(t, c)}<div class="foot">${btn(b[0], 'p', 'sm')}</div></div>`;
...
${mini('出口只有两种：「重试」（问题可能自己好）与「打开大肚猴翻译」（要去 App 里改配置）。能重试的不指去 App，要改配置的不给「重试」—— 给了也是再失败一次。<b>不再有「完成」</b> —— 关闭弹层走系统那个 ✕（为什么删见 SheetFinish 板）。')}`, { page: PG, gap: 16 });
...
${grid(3, `
${cell('选中的只有空白或符号', '不发请求，不报错。', phone(sheet({ engine: '', src: '', lang: null, body: `<div class="trbox"><div class="hint" style="font-size:.9rem">没有可翻译的文字。</div></div>`, foot: '' }), { sel: '· · · — — 2026' }))}
...
${cell('采集开着时', '译文下方一行小字；关采集在 App 里，弹层不放开关（弹层是一次性的，开关该在一个找得回来的地方）。', phone(sheet({ body: trbox(S.zh, copyRow), foot: '', tail: `<div class="disc">这句会存入复习库 · 在 App 的设置里可关</div>` })))}
...
${cell(`iPad ${pend('T1')}`, 'iPad 上系统可能给气泡而不是底部弹层；宽度固定在 420 上下，不铺满。', `<div class="ctx" style="width:520px; height:720px">${host(S.en, 8)}<div class="dim"></div><div class="sheet" style="margin:auto auto 40px; width:400px; border-radius:24px">${header('DeepSeek')}<div class="src">${S.en}</div>${langrow('英语', '简体中文')}${trbox(S.zh)}</div></div>`)}
${cell('深色', '跟随系统；译文用深色的鼠尾草绿 #aebf92，对比度 ≥ 4.5:1。', `<div class="dark">${phone(sheet({ body: trbox(S.zh, copyRow), foot: '' }), { dark: true })}</div>`)}
${cell('阿拉伯语界面（从右到左）', '整个弹层镜像；原文保持自己的书写方向。', `<div dir="rtl">${phone(`<div class="sheet"><div class="grab"></div><div class="sheet-h"><span class="brand"><span class="mark"></span>BelliedMonkey</span><span class="eng">DeepSeek</span></div><div class="src" dir="ltr" style="text-align:left">${S.en}</div><div class="trbox"><div class="tr" style="font-size:1.05rem">${S.ar}</div></div><div class="foot">${btn('نسخ', 'p')}</div></div>`)}</div>`)}
${cell('最大动态字号', '按钮纵向堆叠，译文区可滚；不截断、不缩字。', phone(`<div class="sheet"><div class="grab"></div><div class="sheet-h"><span class="brand" style="font-size:1.2rem"><span class="mark"></span>大肚猴翻译</span></div><div class="src" style="font-size:1.25rem">The committee postponed the vote…</div><div class="trbox"><div class="tr" style="font-size:1.45rem">委员会把表决推迟到新数据公布之后。</div></div><div class="foot" style="flex-direction:column; align-items:stretch"><button class="btn p" type="button" style="font-size:1.2rem; min-height:56px">复制译文</button></div></div>`))}
</div>`, { page: PG });

// 10 · 为什么没有「完成」（2026-09-21 真机定案）
const fin = (host, repl, hit, tapped) => `<div class="old"><div class="c"><b>${host}</b></div><div class="c">allowsReplacement = ${repl}</div><div class="c${hit === 'true' ? '' : ' bad'}">hittable = ${hit}</div><div class="c bad">${tapped}</div></div>`;
board('SheetFinish.dc.html', 1320, 700, '为什么弹层里没有「完成」', `${head('一个点了没反应的按钮，比没有按钮更糟',
  '交互稿原来在每一态底部都画了「完成」，代码里它调的是 <code>context.finish(translation: nil)</code>。2026-09-21 真机（iPhone 14 Pro / iOS 27）逐条验过：<b>两种宿主、两种坏法，同一个按钮</b>。')}
${grid(3, `
${cell('Safari 网页', '译文已落定之后点，三次全不关。2 秒与 6 秒各读一次，六次读回全是「弹层还在」。', fin('Safari（不可编辑）', 'false', 'true', '点了 3 次 · 关掉 0 次'))}
${cell('备忘录', '可编辑，「替换原文」真的出现了（所以 allowsReplacement = true）。但「完成」看得见、点不到。', fin('备忘录（可编辑）', 'true', 'false', '点不到'))}
${cell('结论与改法', '布局逐行读过：按钮就在 HStack 里、在 Spacer() 之后，渲染完全正常 —— 坏的是行为，不是排版。', `<div class="old"><div class="c">✕ 是<b>系统给的</b>，每一轮都靠它关掉</div><div class="c">替换原文 / 复制译文 是真能用的动作</div><div class="c bad">完成 ⇒ 删掉</div></div><p class="mini" style="margin:0">依据是我们自己立过的两条：「灰掉不如不存在」、「不存在的功能不许长死状态条」。</p>`)}
`, 20)}
${mini('<b>我们没有别的出口。</b><code>finish(translation:)</code> 是 Apple 给扩展的唯一关闭入口，而它带 nil 时不工作。所以不是「换个实现」，是这个按钮不该存在。<br>用户 2026-09-20 报的原话就是「点完成没用」——，这一板记的是它的验尸报告。')}`,
  { page: PG, phone: true });
