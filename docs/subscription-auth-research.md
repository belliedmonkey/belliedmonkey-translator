...
⇒ **最终结论（2026-09-09，穷尽之后）：产品不做订阅认证。** 想用订阅的桌面用户，
走一个本机代理（cliproxyapi 之类）+ 我们**现成的自定义端点**即可，凭证与风险全在代理侧、
不在我们这边（§4）。其他用户与 iPhone：免费额度 + 自带 key。**这件事到此 close。**
...
## §4 最终结论：不做订阅认证，交给「本机代理 + 自定义端点」

穷尽四家之后，把整件事收敛成一句话：**我们不该把订阅认证做进产品。**

### 4.1 为什么每条自己做的路都不成立

| 家 | 卡在哪 |
|---|---|
| GLM / Qwen / Kimi Coding Plan | 官方明文禁「自建应用 / 非交互式批量调用」；GLM 连申请入口都没有（§2） |
| ChatGPT / Codex | 可行性来自三重冒充（client_id + originator + 伪造 UA）；后端认不认我们自己的 UA 从未测出结论（见 4.3） |
| Claude 订阅 | **无第三方自助注册入口**（2026-09-09 查实，多来源一致）；2026-02 条款明说订阅 OAuth「仅供 Claude Code 与 claude.ai」；`9d1c250a-…` 是 Claude Code 硬编码的单一 client |
| 官方 Sign in with ChatGPT | 只给身份，不给模型用量（§1.5） |

**没有一家给我们一把属于自己的钥匙。** 而 Hermes 之所以「没问题」，是因为它**根本不注册**、
直接读官方 CLI 登录后的本地凭证（`~/.qwen/oauth_creds.json` 等）—— 借的是官方客户端的身份，
不是它自己的（2026-09-09 查实）。那套在桌面开发者那里成立，在我们这个 iPhone 优先、
全沙箱的形态里既学不了、也仍然是借身份。

### 4.2 已经存在的干净解：本机代理 + 自定义端点

想用订阅的用户，桌面上早有一条干净路，而且**我们一行新代码都不用写**：

- 用户自己跑一个本机代理（cliproxyapi 这类）——它握着订阅认证、暴露一个 OpenAI/Anthropic
  兼容的本地端点。**冒充、扒 token、伪造 UA、ToS 风险，全在代理侧，是用户自己在自己机器上
  的选择。**
- 我们的 `custom_chat` 提供方（本来就支持自定义端点）指向 `http://localhost:<端口>/…` 即可 ——
  对传输层来说它就是一个普通的 OpenAI 兼容网关，和企业网关同一种关系。**我们不持有任何
  订阅 token，不替端点后面的东西背书。**

这正是 domain-design §7「a corporate gateway on its own domain works out of the box」那条
不变量的自然延伸 —— 端点后面是什么，是用户的事。

### 4.3 留给未来的唯一未知数（不建议现在追）

Codex 后端认不认我们**自己的 UA**（而非官方 CLI 的），从未测出结论 —— 那个探针
（`scripts/subscription-auth-probe.js`，分支 `feat/subscription-auth`，**未合并**）因为本机
codex CLI 指向的是 cliproxyapi、读到的 token 不是真的，没跑成。**除非 OpenAI 哪天开放第三方
注册，否则这个未知数没有追的价值** —— 就算 B 通了，登录那步仍要借 Codex 的 client_id。
...
6. **不做订阅认证。** 桌面用户 → 本机代理 + 自定义端点（§4.2，零新代码）；
   iPhone 与其他用户 → 免费额度 + 自带 key。可选：写一小段文档，告诉跑代理的用户
   「在自定义端点里填你本机代理的地址」——那只是把现有功能讲清楚，不碰任何凭证。
7. `feat/subscription-auth` 分支（探针）**不合并**，留作记录。这件事 close。
