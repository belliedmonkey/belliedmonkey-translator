...
> **这一节是提案，不是现状。** 评审通过之前，`grant.china` 仍是 `false`、中国合规门
> 那三条禁令仍然有效、客户端仍然要用户点一次「领取」。交互稿：
> https://claude.ai/artifact/PTAr3ymseUw7s1sKvUX7Kf
...
1. **百炼的条款。** 我们是**替用户转发**而不是用户自己调 —— 「代他人调用 / 转售」这类条款
   要单独确认。（与「订阅套餐禁止自建应用 / 非交互式批量」是两码事：那条针对订阅套餐，
   不针对 API 计费。）
2. **出境告知与单独同意必须同版落地**，见下一条 —— 身份线仍然出境，而中转会让境外接收方
   多一个。**不能先发额度、后补文案**（`growth-spec` §4）。
