...
- **「我说」= 按住说话（walkie-talkie），松手后留在本页**（用户 09-07 review 裁定）：按住期间
  麦克风听到的都算我的（对方在这个窗口里说的话接受漏掉）；松手 ⇒ 这段转写为中文 → 译成外语，
  历史加一行「我：」，行下有「朗读」（只在有 TTS 引擎时存在，**由用户点**）与「给对方看」。
  暂停/停止态下也能直接按住（它自己把麦克风起起来）。
- **给对方看 = 点任一历史行放大**：全屏展示卡（外语大字 + 中文小字 + 朗读/关闭），底下照常在听；
  卡上不放星（星在行上）。
- **不可用的样子只有一种**（45% 透明、文案不变），且屏上一定有原因；「按住 · 我说」只在准备中、
  按住时（暂停键）、麦克风被拒绝、socket 重连中四种情况下灰。socket 断开先自动重连一次（2 s）。
