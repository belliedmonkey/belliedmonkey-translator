...
- **原生语音处理是升级，不是前提**（2026-09-08）：麦克风会话开启
  `setVoiceProcessingEnabled(true)` —— 一个开关同时给到**回声消除 + 噪声抑制 + 自动增益**，
  后两样正是「嘈杂环境听不清」的答案。**位置是硬的**：必须在读输入格式、装 tap 之前，
  晚一行转换器和 tap 就都按旧格式建，表现是「权限给了、tap 装了、一块 PCM 都不出来」。
  失败**静默降级**（没有它的听译仍是完整功能，而「麦克风启动失败」是四种停止态里最贵的
  一种），事实位取 `isVoiceProcessingEnabled` 的**读回值**，随 `granted` 一起上报给 JS。
  iOS 17 起把「压低其他音频」调到最轻，否则对方听到的朗读忽大忽小 —— 而那个「其他音频」
  正是我们要读给对方听的译文。**效果尚未在真机量过**（H1 尖刺，learn-regression M25）：
  消不掉也不改变任何产品行为，上面四道闸本来就是地板。另有一条运行期开关 `mic-aec`
  只为尖刺存在 —— 两次会话之间房间与增益都会变，只有同一次会话里翻转才叫对照。
