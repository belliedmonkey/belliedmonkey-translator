...
- **驾车模式（§9.5）不参与本节，一行都不参与**：它是播放器，只曝光材料，
...
## 9.5 驾车模式 (driving mode) — App 卡片播放器（2026-08-17，2026-08-18 重定位）
...
- **前台 only**。App 无原生音频会话，锁屏即停；屏幕常亮由 `app:sync` 的
  `isIdleTimerDisabled` 补丁保证。锁屏/后台播放（原生桥 + `UIBackgroundModes`）
  整体后置（§12）。**App 专属**：iOS Safari 扩展页拒绝无手势播放（§9.1 实测），
  连续播放在那里不可能存在——按能力语义，驾车模式不进扩展任何字节。
...
开启后，每张卡在原文和译文之后再读一遍解析（生词 / 短语 / 一个语法点，§9.2 的产物）。
**没解析过的卡会当场解析**——调用用户自己配置的解析引擎。

- §9.2 的成本合同不变：`LearnNotes.get` 缓存优先，**每张卡至多收一次费，永远**。
  新的地方是**驾车会话可以成为触发那一次收费的东西**，而且是在手机架在支架上、
...
- 解析为空 → 跳过这一段（**不播空音频**：那会让界面停在一个和「卡住了」完全一样的
  ...
  不是终止整场的理由。
- 解析引擎没配置（§9.2 能力门关着）→ 这一段**不存在**，不是灰的。

### 中断与失败具名（§9.1/§9.4 reason 惯例）

**失败要分得开**，这是这一节唯一容易做错的地方：

- **整机级失败**（自动播放被拦 `blocked`、不支持语音 `unsupported`）→ 停在具名原因上。
  它会在每一张卡上重演，继续下去只是空转。
- **单卡级失败**（这张卡的语言没有音色 `no_voice` / `no_voice_und`）→ 说一声，跳到
  下一张。一张读不出来的卡不该毁掉一场其余卡都能读的会话。
- 两者都**绝不静默**：一张卡无声无息地消失，读起来就像数据丢了。
- `visibilitychange` 隐藏（锁屏、来电、切 App）→ 暂停并停 TTS，**恢复只能手点**——
  一辆自己重新开口说话的车比一辆等着的更糟。暂停记住停在哪一段，继续时从那一段重来。
...
| 2026-08-18 | 驾车模式里的跟读题（免提推进真实学习进度） | **录音窗口的存在必然打断连续播放**，而连续播放是「开车时听」的全部内容。一个每张卡都停下来等你说话的播放器不是播放器。学习层的写路径全部留在复习页——那里用户看得见自己在给什么打分（§9.5） |
| 2026-08-18 | 驾车模式里的语音问答（卡末「有没有疑问？」） | 同上：它要求一个录音窗口，且把一次被动收听变成一连串需要应答的提示。免提问答本身仍是好想法，但它属于一个独立的语音助手形态，不属于播放器（§9.5） |
