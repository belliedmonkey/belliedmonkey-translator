...
25. **借系统后退 / 前进按钮翻看字幕历史**（用户：「能不能手动滚动字幕历史」；小窗不接受触摸滚动）。
    ...
    - **按按钮上的秒数翻**（2026-09-14 同日修订，原为「每次 3 句」；用户：「后退前进按钮能不要搭配一个数字吗 这个数字没什么意义」「如果翻3句就显示3吧？」）：按钮图标与秒数由系统挑选，App 无接口改（iOS 26.5 SDK 的 `skipByInterval` 只把秒数告诉 App），显示不了「3」⇒ 反过来让数字名副其实：⏪10 = 回到 10 秒前说的那句，⏩10 = 往新翻 10 秒；秒数一律取系统传来的值；那段时间里一句都没有时至少翻一句。原生给每句定稿记到达时刻。
    - 翻看时顶部一行 `labels.pip.history`；新句子到来不把画面拽回最新。
    - 原生本地状态，不回页面。
    - 待真机：按钮是否出现、进度条是否碍眼。
