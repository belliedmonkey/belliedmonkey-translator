...
| 日期 | 评审人 | 范围 | 结论 |
|---|---|---|---|
| 2026-09-17（晚，待人评审） | belliedmonkey | **离线朗读模型的下载地址改为后端配置表决定**（用户提议「地址存入数据库，每次从数据库获取，以后地址不能用了还可以换」）。清单 `app/device-models.config.js` 继续钉 **sha256 与 size**（服务器只能说「去哪下」，不能说「下什么」——换了文件过不了校验照旧丢），`url` 变成兜底默认值；表里每个文件放**默认 + 备选**两个地址，默认失败自动换备选，都失败再回清单；下载前向 `bt_model_sources` 表问一次当前地址（匿名 GET，不带任何个人数据，5 s 超时，24 h 缓存），问不到就用清单里的。动因：09-17 真机实测境内拉不动 GitHub，换托管每次都要发版；同日建了两处我们自己的托管：ModelScope `belliedmonkey/belliedmonkey-device-models`（真机 18.7 MB/s，中国版默认）与 Hugging Face 同名仓库（经 hf-mirror ≈580 KB/s，备选）；Supabase 公开桶也可用但按流量计费，只作最后备选。不进包（用户同日收回「存包」：不同语言要不同模型）。§9.6.1 新增 §9.6.1.1；§10 Gate H 隐私文案补一句。 | 待评审 |
...
**下载顺序（一次「下载」按钮 = 最多三次尝试，用户只看到一个进度行）：** 表里的 `url` → 表里的 `url_alt` → 清单默认值
（与前两者相同则跳过）。每次失败后进度行写一句「默认地址失败，换备用地址重试…」（`tts_pack_fallback`），进度归零重来；
**三处都失败才进「下载失败」态**，`{why}` 记最后一次的原因。切换在 JS 侧做（`ensureDeviceReady` 收到 `{reason:'download'}` 后换地址
再调一次 `ensureAssets`），原生的下载器不用知道有几个地址。清单里也允许每个文件带 `urlAlt`（同样按 flavor 分），作为表不可达时的第二兜底。
...
24 h 内走缓存、**server 返回的 sha256/size 字段被忽略**；下载顺序：默认失败 ⇒ 备选，备选失败 ⇒ 清单，三处失败才是失败态，
且三次尝试用的是**同一个** sha256。`test:listen`：假后端返回覆盖地址 ⇒ 原生收到的
