...
| M14 | **出发前预载 → 断网整轮播完**（§9.5）**✅ 2026-08-23 模拟器实证** | iPhone App | 实测通过（iPhone 15 Pro · iOS 17.2，真账号 862 张卡、当日牌库 20 张，OpenAI Speech + DeepSeek 真端点）：账单 20 张卡 · 待合成 61 段 · 待解析 20 张 ⇒ 第一下**零消费**、第二下跑完「完成 20/20」、缓存读数 61 条 ≈18 MB（**预测数与实际数一致**）；随后把两个端点都指向死地址（`cacheKey` 不含 baseUrl，所以缓存全部命中而任何真发出的请求都会响亮失败）⇒ 连播到第 2 张、第 3 遍，解析从缓存渲染，无空档无报错。**断网用「端点指向死地址」而不是飞行模式**：飞行模式证明不了「没发请求」，只证明「发不出去」。第二次预载的账单降到 61（待解析 0）—— 顺带实证了 §9.2「每张卡至多收一次费」 |
