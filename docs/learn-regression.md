...
| M22 | **锁屏 60 s 继续听**（§9.6 原生采集） | iPhone App（真机） | 开始听 → 锁屏（镜像下 = 退出镜像）→ 对着手机继续念 60 s → 解锁：历史里多出锁屏期间的句子，读数面板 `while hidden: frames` 持续增长、finals > 0（PR-L0 在页内采集下这两个数恒为 0，这条是它的证伪面）；锁屏卡片 = 最近一句定稿 + ⏸ ■；锁屏上按 ■ ⇒ 会话真结束。**反面**：麦克风被系统收回（来电）⇒ 停在「锁屏后系统停止了录音」或「转写连接中断」具名态，不是静默 |
| M23 | **我说 → 翻面 + 朗读** | iPhone App（真机）+ macOS App | 按住「我说」说一句中文（按住期间对方的 PCM 不发：网络日志里无 `input_audio_buffer.append`）→ 松手：翻面大字外语 + 中文小字 + 「朗读中」真出声（TTS 引擎已配）；换成无 TTS 引擎 ⇒ 只显示不朗读、无道歉文案 → 「继续听对方」回到听 → 历史多一行「我：」。macOS：按住空格等价于按住按钮 |
