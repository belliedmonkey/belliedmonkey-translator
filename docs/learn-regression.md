...
| 驾车模式 | `learn-driving.test.js` | §9.5：播放顺序（随机是**排列**不是抽样、绕圈重洗、顺序播放会结束而其余三种不会、下一张在单曲循环里也能跳走）、`cardPlan` 分段（默认不含解析）、`notesToSpeech`（缺项少一段、无内容返回空串供调用方跳过）、`reduce` 走查（原文→译文→**直接推进**、解析段取→播→推进、整机级失败停下 vs 单卡级失败跳过、暂停记住段号、换模式不打断音频）、**以及反向断言：模块表面没有任何写进度的函数，reduce 产生的效果里没有任何写操作** |
...
| 10a | 驾车自动连播 + **零写入** | 顺序播放走完一整轮：原文紧跟译文、播放段数 ≥ 卡数×2（每张都被自动播到，中间无等待）；跑完 IndexedDB **零新复习行**、`sched`/`skills` 一字不动（load-bearing 反向断言 —— 这个模式只曝光） |
...
| 10d | 播放解析 + 自动补全 | 清空所有 notes 后开启开关：未解析的卡触发解析请求、解析文本（生词+译义）被 speak、播放中费用提示常驻；且该会话仍**零复习行** |
| 10e | 驾车暂停 | 暂停 ⇒ `LearnTTS.stop` 被调，此后零写入；恢复须手点 |
...
| M11 | 驾车连播真跑（§9.5） | iPhone App | 进驾车模式 → 多张卡连续原文/译文朗读**无需逐句手势**（tts.js 的 epoch/settle 只被逐点播放验证过，长链是新面）；四种播放顺序各切一次、行驶中按模式键不打断音频；整机级 TTS 失败停在具名原因上，单卡级失败跳过并说明；开启播放解析后，未解析的卡当场补解析并读出 |
| M12 | ~~TTS→麦克风交接~~ | — | **已作废（2026-08-18）**：跟读题从驾车模式移除，麦克风完全不参与这个模式。音频会话交接不再是这条路径上的风险面 |
| M13 | 屏幕常亮 + 音频路由 | iPhone App | 驾车会话中屏幕不自动锁（idle-timer 补丁生效）；蓝牙/CarPlay 路由下声音走车机（sanity，不追求完美）；来电/切后台落在暂停态，恢复须手点 |
