...
| M11 | 播客连播真跑（§9.5） | iPhone App | **2026-08-26 起解析是逐行念的**（生词/短语/语法三块），所以「三遍节奏」现在多出两个 utterance 边界 —— 听下来若一顿一顿，**先退拆分粒度**（三块→整段），不是砍掉封面高亮。进播客模式 → 多张卡连续朗读**无需逐句手势**；**三遍节奏的听感**（会不会太啰嗦——若过头先退第三遍）；四种播放顺序各切一次、行驶中按模式键不打断音频；**解析引擎没配时那句说明真的出现**（2026-08-17 漏掉的正是这个场景）；暂停 → 「解析这句」 → 读完回到暂停；整机级 TTS 失败停在具名原因上，单卡级失败跳过并说明 |
...
| M16 | **后台 / 锁屏 / 遥控**（§9.5「后台与锁屏播放」）**⚠️ 模拟器已跑，真机待验** | iPhone App | **已在模拟器上跑过①的形状**（iPhone 15 Pro · iOS 17.2，2026-08-24）：顺序播放下后台 120 秒，第 1 张 → 第 3 张（端点语音走本地缓存，纯离线）；把门临时打开后，**设备内置语音**后台 240 秒、第 1 张 → 第 6 张 —— 详见 verification-spec 的尖刺表。**模拟器的绿是弱证据**，②～⑥ 全部仍需真机。另已实证：设备内置语音下常驻行如实显示「设备内置语音在 App 退到后台或锁屏后会停下来；想放着听，请配一个语音端点（设置 → 语音引擎）」，即⑥的正面。<br>真机步骤：前置：跑一次「出发前预载」，让整轮零网络（把「后台还能不能发请求」这个变量消掉）。① 进播客模式 → 按 Home 退后台 → **60s 后仍在播**，回前台后段序连续（事件日志/`_debug()` 为证）；② 锁屏 → **仍在播**；锁屏卡片显示原句 + 译句 + 「第 i / n 张 · 第 k 遍」，**没有进度条**；③ 锁屏上点暂停/播放 → 真停/真续；「下一曲」→ 换卡；**「上一曲」→ 回第一遍开头**（不是上一张）；④ 蓝牙耳机/车机：连上后按播放键 → 续播；**拔出耳机 → 暂停且不会外放**；⑤ 来电 → 暂停；挂断 → **自动续播**（本次新规约的判据）；⑥ **反面**：换成不能后台播的引擎 → 退后台**如实暂停**，且常驻行上有一句话说明是哪一条不成立、去哪儿改 —— 没有这一条，①的绿可能只是「反正也没在听」。<br>**④⑤ 驱动不了**（镜像占蓝牙，且镜像期间没法真锁屏，见 verification-spec §0.2）：照既有纪律请用户听 + 拍照，别假装自动化跑过。②可以用**控制中心的 Now Playing 小组件**当锁屏卡片的可驱动代理 —— 它读的是同一份 `MPNowPlayingInfoCenter` 数据 |
...
| M19 | **锁屏封面与灵动岛**（§9.5）**⚠️ 锁屏已在模拟器实证，灵动岛真机待验** | iPhone App | **2026-08-25 模拟器锁屏实证通过**（iPhone 15 Pro · iOS 17.2）：封面显示我们画的卡片（白卡 + 橙色进度胶囊）、标题是卡片原句、副标题是译句、进度位置显示「实时」而非拖动条、⏪10/⏩10 已消失。**这一条是把 `MPNowPlayingInfoCenter` 改成 `navigator.mediaSession` 之后才成立的** —— 改之前锁屏上显示的是页面标题「大肚猴翻译 · 复习」。<br>**灵动岛仍需真机**（模拟器对灵动岛支持不完整，镜像会占掉它的位置，两者都不可信）。<br>其余真机步骤：① 锁屏上出现封面，**看得清正在念的那一句**（长句子在媒体控件那一行必然被截，封面是唯一读得完的地方）；② **灵动岛上是大肚猴图标，不是问号** —— 这是本轮的验收判据；③ 切换系统深/浅色，下一张卡的封面跟着变；④ 裸卡（没有译文的卡）封面下半张不留空白；⑤ 封面上**没有**遍次（它每张卡只画一次，遍次在卡内会变），遍次只在媒体控件那一行；⑥ 没有进度条（`IsLiveStream`）。<br>**⑦ 顺带量一件没有文档的事**：读 `AppDriving._debug().bridge.artSizes` —— 系统实际用哪些尺寸来问封面，Apple 没有公开。锁屏 / 灵动岛 / 控制中心各走一遍再读，用实测值校准 `iconMaxWidth`（现在的 120pt 是初值）。**这个数不量就是猜，猜错的表现是灵动岛依旧糊。**<br>已在真 Chrome 里验过的部分（`test:learn`）：封面画得出来、是合法 data URL（没被 canvas 污染挡住）、1024² PNG ≈123 KB、同一张卡只推一次。真机要验的是**系统怎么用它** |
| M20 | **灵动岛 Live Activity**（§9.5）**⬜ 真机待验** | iPhone App（真机，iOS 16.1+） | **只能真机验**：模拟器不渲染灵动岛，iPhone 镜像自己占着灵动岛的位置 —— 两者都不可信。① 进播客模式 → 退到主屏 → **收起态显示大肚猴 + 「第 i / n 张」**，不是问号；② 长按展开 → 原句 + 译句 + 进度；③ 暂停/继续在岛上立刻看得出来；④ 退出会话 → 岛上那一条**当场消失**（不留一条不动的信息）；⑤ 系统设置里关掉「实时活动」→ 不报错、不提示，其余功能照常（能力语义）；⑥ **Live Activity 与系统媒体形态并存，岛上会挤** —— 看一眼实际观感，难看到不可接受就退回只留系统形态。<br>模拟器上唯一能确认的（已做）：系统日志里 `chronod` 真的拉起了 `MTPodcastWidget` 进程，说明注册成功、扩展被加载。外观一概不算。 |
