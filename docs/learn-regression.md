...
| 4 | und 卡 | **2026-09-13 修订**：可用性判断带卡文本，英文文本按脚本兜底成英语语音 ⇒ 播放**可用**、不说「语言未知」；被看到后 `langAsked` 落盘（脚本猜不出的拉丁字母问一次翻译引擎写回 `lang`，假端点答非语言码则保持 und）。旧判据「播放禁用、提示指路到我们的语音设置」只对**没有任何同脚本系统语音**的卡成立（learning-design §4.1 修订） |
...
| M21 | **实时听译端到端**（§9.6）⬜ 真机待跑 | iPhone App（真机） | 首页入口存在（转写引擎带实时接口且 key 已填；换成无实时接口的引擎 ⇒ 入口消失、指路句出现）→ 开始听 → **原生**权限弹一次 → 对着手机念一段外语 60 s → 上卡逐词出现、历史出整句、译文在定稿后 ≈ 1–2 s 到 → 加星一句 → 结束 → 小结数字与历史一致 → 复习页出现该句（来源「对话」，ⓘ 行显示会话标题，**没有 ▶**）→ 来源管理里该会话一行、「删除已存」整段删。**模拟器只验入口门控与界面**（麦克风 0 字节） |
| M22 | ~~**锁屏 60 s 继续听**（§9.6 原生采集） | iPhone App（真机） | 开始听 → 锁屏（镜像下 = 退出镜像）→ 对着手机继续念 60 s → 解锁：历史里多出锁屏期间的句子，读数面板 `while hidden: frames` 持续增长、finals > 0（PR-L0 在页内采集下这两个数恒为 0，这条是它的证伪面）；锁屏卡片 = 最近一句定稿 + ⏸ ■；锁屏上按 ■ ⇒ 会话真结束。**反面**：麦克风被系统收回（来电）⇒ 停在「锁屏后系统停止了录音」或「转写连接中断」具名态，不是静默 | **（2026-09-17 作废：云端实时档下线，本机路由 M24 / M25 覆盖）**
| M23 | ~~**我说 → 翻面 + 朗读** | iPhone App（真机）+ macOS App | 按住「我说」说一句中文（按住期间对方的 PCM 不发：网络日志里无 `input_audio_buffer.append`）→ 松手：翻面大字外语 + 中文小字 + 「朗读中」真出声（TTS 引擎已配）；换成无 TTS 引擎 ⇒ 只显示不朗读、无道歉文案 → 「继续听对方」回到听 → 历史多一行「我：」。macOS：按住空格等价于按住按钮 | **（2026-09-17 作废：云端实时档下线，本机路由 M24 / M25 覆盖）**
| M26 | **Mac 系统声音字幕**（§9.8）⬜ 待实现 | macOS App（真机） | 判据见 verification-spec F-bis M26：Safari MSE 视频出双语字幕、说完 ≤ 3 s 上条、换耳机输出仍出 |
| M27 | **悬浮条层级与焦点** ⬜ | macOS App | 盖在全屏视频 / Zoom 上、不抢焦点、位置记住 |
| M28 | **Mac 关主窗口不停** ⬜ | macOS App | 隐藏 10 min 节奏差 ≤ 20%，Dock 找回历史完整 |
| M29 | **Mac 系统版本门** ⬜ | macOS 13.x | 入口灰 + 具名原因 |
| M30 | **系统录音权限被拒** ⬜ | macOS App | 条上具名 + 打开系统设置 |
| M31 | **iPhone 后台听外放** ⬜ | iPhone App（真机，国际版 + 中国版） | Safari 不被暂停、音量不被压；画中画小窗离开 App 自动浮出并随句滚动（机制已由尖刺 S7 真机证实，产品实现后复测） |
| M32 | **iPhone 屏幕录制 + 耳机**（二期）⬜ | iPhone App（真机） | 戴耳机出字幕；扩展内存 < 25 MB；未布防即结束 |
