...
| macOS Safari | ◐ 侧载成功、扩展面板可验；**页面渲染无法通过 cua 截图观察** |
| macOS Chrome | ✅ 跑过（抓到 3 个缺陷，均已修） |
| Firefox | ◐ 结构通过；翻译链路 + 探测器请求检查待补 |

按 §0，**这不构成一次完整验证**。上表就是「诚实说明哪些跑了、哪些没跑」本身。

### macOS Safari —— 侧载成功，页面渲染无法用现有手段观察

**两处与旧笔记不符，已更正：**

1. **「允许未签名的扩展」这次已经是勾上的**（距上次授权多日、Safari 中间退出过多次），
   **没有再要密码**。旧结论「该勾选是 session-scoped、每次都要重新授权」不成立。
   **以后先读 AX 里那个复选框的 value 再决定要不要麻烦用户。**
2. **真正卡住 agent 的不是密码，是「添加临时扩展…」按钮本身**：AX `AXPress` 返回
   -25204，前台像素点击也不弹面板——文件夹选择器由 macOS 沙盒的 Open/Save Panel
   Service **出进程**托管。§2.C 把「选文件夹」标为人工步骤是对的，而且实测它是**唯一**
   的人工步骤。

**已验证**：临时扩展载入并启用 · 版本显示 **1.3.0** · 描述正确本地化 · 「开发」菜单出现
「网页扩展背景内容」（仅在载入网页扩展后才有）。

**未能验证**：页面上的任何渲染行为。Safari 的网页内容在独立进程中渲染，`get_window_state`
的窗口截图**始终是空白**，而标题栏显示页面已加载。这不是「页面空白」的证据，也不是「渲染
正确」的证据——**它什么都不是**，所以本行的 FAB / 翻译 / 采集 / 复习页 / TTS 一律记为
**未跑**，而不是通过。
