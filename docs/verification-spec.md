...
| **M31** | iPhone 后台听外放 | App 开始 → 切 Safari 外放 lab-server 页的 `conv.wav`，再放一个 YouTube 视频，共 5 min（先退出 iPhone 镜像） | Safari 不被暂停、音量前后 ±3 dB（Mac 麦克风测）；定稿持续；灵动岛与锁屏卡随句更新；**国际版 + 中国版**各一遍 |
