...
| iPhone / iPad Safari (≥ iOS 17.1) | same as macOS Safari; the AudioContext is created inside the tap handler | **expected silent for both MSE and native HLS** after the macOS measurement above (same WebKit); the `blob:` check gives the named stop, native HLS (`src=.m3u8`) falls to the silence guard, which must report within 3 s, never a stuck 「实时转写中」 | same; screen lock during a live session ⇒ 「转写连接中断」 on return, not silence |
