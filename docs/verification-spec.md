...
| **M26** | Mac 系统声音字幕 | App「实时字幕」开始 → Mac `afplay conv.wav`；再用 Safari 播一个 MSE 视频（YouTube，扩展在 Safari 里静音的那格） | ≥ 11/12 句定稿；说完 ≤ 3 s 悬浮条出双语；把输出切到耳机再跑一遍仍出字幕；记 CER 与 `lat.pass`（数字声音应优于麦克风读数） |
