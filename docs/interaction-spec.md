...
**iPhone 一期 · 进行中**（I3–I10）。开始后页面提示「切到 Safari 点播放吧（外放）—— 字幕会出现在灵动岛和锁屏上」；若视频原本在放，切回 Safari 后要再点一次播放。灵动岛收起态：猴子 +「{n} 句」+ 绿点（放不下句子，I4）；长按展开：译文 2 行 + 原文 1 行 +「实时字幕中 · {t} · {n} 句」+ 暂停（I5）；锁屏 / 通知中心卡片同内容（I7 / I8）；回 App 看完整历史（I9）。「每句无声短暂展开」待真机尖刺（I6），不行就维持长按。停机（I10）各带出口：听不到视频声音（戴耳机）· 30 秒没有声音 · 麦克风被拒绝 · 录音被系统停止了 · 转写连接中断。
