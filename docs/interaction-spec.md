...
**准备页**（M3 / I2 / B1）。「视频的语言 → 我的语言」摊在外面（理由同对话的语言对）；转写 / 翻译引擎一行；「字幕进复习（来源『实时字幕』）」默认开、「这次不留记录」默认关（开始即钉住）；主按钮「开始」；费用行；Gate I 隐私段。iPhone 多一张提示卡「先点开始，再切到 Safari 外放播放视频；字幕显示在灵动岛和锁屏上。戴耳机时听不到视频声音。」（顺序是尖刺 S5 定的：Safari 一离开前台 iOS 就暂停网页视频，所以先放视频再切来 App 开始，视频会停）二期多一个次按钮「用屏幕录制方式开始 · 戴耳机也能用」，隐私段换成屏幕录制版本。
...
**iPhone 一期 · 进行中**（I3–I10）。开始后页面提示「切到 Safari 点播放吧（外放）—— 字幕会出现在灵动岛和锁屏上」；若视频原本在放，切回 Safari 后要再点一次播放。灵动岛收起态：猴子 +「{n} 句」+ 绿点（放不下句子，I4）；长按展开：译文 2 行 + 原文 1 行 +「实时字幕中 · {t} · {n} 句」+ 暂停（I5）；锁屏 / 通知中心卡片同内容（I7 / I8）；回 App 看完整历史（I9）。「每句无声短暂展开」（I6）：模拟器尖刺 S6 已证实每句定稿用带提醒的更新会自动展开约 5.5 秒、普通更新不会；是否限流、能否无声不震要真机确认，不行就维持长按。停机（I10）各带出口：听不到视频声音（戴耳机）· 30 秒没有声音 · 麦克风被拒绝 · 录音被系统停止了 · 转写连接中断。
