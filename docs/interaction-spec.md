...
**iPhone 一期 · 进行中**（I3–I10）。开始后页面提示「切到 Safari 点播放吧（外放）—— 字幕会浮在画中画小窗里」；若视频原本在放，切回 Safari 后要再点一次播放。**iPhone 上「现在」卡换成小窗预览**（2026-09-14，learning-design §9.8 协议补充决定（三）17）：与画中画同一帧的 16:9 深色小图、最近两三句 —— 系统要求画中画的来源就在屏幕上，离开 App 时由它浮出；小窗被 ✕ 关掉后，预览下方出「浮出字幕窗」按钮。**离开 App 时自动浮出画中画小窗**（I4；尖刺 S7 真机 1.7 秒内出现）：深色底，最近两三句往上滚，译文粗体在上、原文小字在下，最新一句在最下面，半句用斜体。捏合放大一次看 3–4 句，按系统回报的新尺寸重画（I5）；拖到任意角落，滑到屏幕边缘收成把手；点一下出系统控件：✕ 关小窗（不停止听译，回 App 点「浮出字幕窗」再打开）、⧉ 回到 App（Safari 视频会被系统暂停）、⏸ 暂停听（与 App 里的暂停同一个）（I6）。锁屏时系统不显示画中画，照常听，解锁后小窗回来（I7，待真机验证）。停下时小窗不消失，最后一句留着、底部换成一行原因（I8）。回 App 看完整历史（I9）。**不再做灵动岛 / 锁屏卡**（2026-09-13 用户看真机：「这个灵动岛展示没什么意义，字小看不清」；专注模式开着时根本不展开）。停机（I10）各带出口：听不到视频声音（戴耳机）· 30 秒没有声音 · 麦克风被拒绝 · 录音被系统停止了 · 转写连接中断。
