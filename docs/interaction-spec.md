...
| Safari and the source is `blob:` (MSE — YouTube, Twitch, X) | 「Safari 抓不到这类流媒体视频的声音，此页无法转写」 — before any socket opens (measured 2026-09-07: WebKit's `createMediaElementSource` is silent for MSE while the element stays audible) |
