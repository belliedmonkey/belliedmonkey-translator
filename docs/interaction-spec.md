...
**准备页**（M3 / I2 / B1）。「视频的语言 → 我的语言」摊在外面（理由同对话的语言对）；转写 / 翻译引擎一行；「字幕进复习（来源『实时字幕』）」默认开、「这次不留记录」默认关（开始即钉住）；主按钮「开始」；费用行；Gate I 隐私段。iPhone 多一张提示卡「先点开始，再切到 Safari 外放播放视频；字幕会浮在画中画小窗里。戴耳机时听不到视频声音。」（顺序是尖刺 S5 定的：Safari 一离开前台 iOS 就暂停网页视频，所以先放视频再切来 App 开始，视频会停）二期多一个次按钮「用屏幕录制方式开始 · 戴耳机也能用」，隐私段换成屏幕录制版本。
...
**字幕条状态**（M6，都写在条上）。正在听系统声音… · 半句 · 定稿 · 已暂停 · 继续 · 30 秒没有声音，已暂停以免计费 · 继续（不自动恢复）· 听不到系统声音 — 请到 系统设置 › 隐私与安全性 › 屏幕与系统录音，允许「大肚猴翻译」· 打开系统设置 · 转写连接中断 · 正在重连… / 重试 · 正在下载{语言}识别资产 · {pct}%。某句翻译失败（M10）：条上只显示原文 +「这句译文失败 · 主窗口里可重试」，主窗口那一行「译文失败 · 重试」。

**iPhone 一期 · 进行中**（I3–I10）。开始后页面提示「切到 Safari 点播放吧（外放）—— 字幕会浮在画中画小窗里」；若视频原本在放，切回 Safari 后要再点一次播放。**iPhone 上「现在」卡换成小窗预览**（2026-09-14，learning-design §9.8 协议补充决定（三）17）：与画中画同一帧的 **4:5** 深色小图（2026-09-14 用户手测后改为偏竖）—— 系统要求画中画的来源就在屏幕上，离开 App 时由它浮出；小窗被 ✕ 关掉后，预览下方出「浮出字幕窗」按钮。**原生预览没叠上来的时候（开始前 / 结束后），占位块上写一句说明**：开始前「点『开始』后，这里是字幕小窗的预览；切到 Safari 时它会浮在上面」，结束后「这次字幕已结束，小窗已关闭」—— 不是一块黑（2026-09-14 用户 15 Pro 手测：「这个地方都黑屏很奇怪」）。**离开 App 时自动浮出画中画小窗**（I4；尖刺 S7 真机 1.7 秒内出现）：深色底 4:5，最近四五句往上滚，译文粗体在上、原文小字在下，最新一句在最下面，半句用斜体。**还没有一句字幕时，小窗画三行说明**（「实时字幕」·「在 Safari 里外放视频，字幕会出现在这里」·「点一下小窗：⏸ 暂停 · ⏪⏩ 翻看历史 · ✕ 关掉（回 App 可再打开）」），不是一块黑（2026-09-14 用户：「最小化 app 后立刻出现了这个黑色小窗，用户很莫名其妙」）。捏合放大按系统回报的新尺寸重画（I5）；拖到任意角落，滑到屏幕边缘收成把手；点一下出系统控件：✕ 关小窗（不停止听译，回 App 点「浮出字幕窗」再打开）、⧉ 回到 App（Safari 视频会被系统暂停）、⏸ 暂停听（与 App 里的暂停同一个）、**⏪ / ⏩ 翻看历史**（每次 3 句；翻看时顶部标「历史 · 点 ⏩ 回到最新」，learning-design §9.8 协议补充决定 25）（I6）。锁屏时系统不显示画中画，照常听，解锁后小窗回来（I7，待真机验证）。停下时小窗不消失，最后一句留着、底部换成一行原因（I8）。回 App 看完整历史（I9）。**不再做灵动岛 / 锁屏卡**（2026-09-13 用户看真机：「这个灵动岛展示没什么意义，字小看不清」；专注模式开着时根本不展开）。停机（I10）各带出口：听不到视频声音（戴耳机）· 30 秒没有声音 · 麦克风被拒绝 · 录音被系统停止了 · 转写连接中断。
