...
## 免费额度 — 登录领一把封顶 0.2 美元的 key（2026-09-08）
...
- **必须登录才能用**（用户裁定）。退出登录的动作本身清掉本机三槽里尾号 = `grantTail` 的
  key 与 `grantTail` / `grantBalance`，只留 `grant` 记录；弹窗与页面回到「未配」那条；
...
- **「你用完了」与「我们的池子空了」是两句话。** 402 先强刷余额：余额还在 ⇒ 状态
  `unavailable`，卡上明写「不是你用完了」，只给社群出口；用完 ⇒ 两条路并列，「一键配置
  自带 key ↗」（注册表 `keyUrl`）在前、「加入社群寻求帮助 ↗」（`COMMUNITY_URL`）在后。
- **粘了自己的 key 就是自己的 key。** 免费槽（尾号命中）可覆盖，结果行如实写「替换免费额度
  的 key」；卡上给「改回免费额度」（`LearnDialog.confirm`，会覆盖现有 key）。在「详细」里
  改了模型 ⇒ 白名单 403 ⇒ 一句「免费额度只能用 {model} — 改回」，只写 `apiModel`。
- **额度不含实时听译**（OpenRouter 转写没有实时接口）。三处提前说：引导左卡的转写行、
