...
//
// ── 表外还有一个出口，但它不改变上面的规则 ─────────────────────────────────
// #162 起，用户可以在设置页填一段自定义 JSON，按引擎存、合并在最后（**用户覆盖表**）。
// 那是给私域端点的：企业网关、自建服务，我们永远测不到，而用户手上有那份知识。
// 它**不放宽这张表的准入**：没填自定义时，表外 host 的请求体仍然只有 {model, messages}。
// 换句话说，「不知道就少说话」仍是默认态；逃生口是显式的、按引擎的、后果自负的。
// 所以用户报「某个私域端点慢/失败」时，第一反应应该是让他试自定义参数，
// **而不是**急着往这张表里加一行没有实测证据的 host。
//
