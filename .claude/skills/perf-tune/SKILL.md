...
## 用户报「我的端点慢/失败」时，先问的不是这张表

私域端点（企业网关、自建服务）**我们永远测不到** —— 没 key、打不到、也不该要用户交出
内网地址。设置页的「自定义请求参数」就是给他们的：按引擎存、合并在最后、用户覆盖表。

所以顺序是：

1. 让用户在自定义框里填他知道的字段（比如 `{"thinking":{"type":"disabled"}}`），
   面板会当场显示「将发送：…」与「不可覆盖，已忽略：…」。
2. **只有当那个 host 我们真的能打到、且实测出结论时**，才往 `model-params` 加一行。
3. 加不进表也不是失败 —— 表外 = 最小必要 = 不会被拒，用户那条自定义参数照样生效。

⚠️ 反过来说：**不要因为一个用户报了问题，就往表里加一行没有实测证据的 host。**
那正是这张表的许可证禁止的事。
