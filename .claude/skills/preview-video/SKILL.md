...
| runner 起不来 `Developer App Certificate is not trusted` | **装了新 App 的二进制**（不是重建 runner）⇒ 同一开发者身份签的程序一起被挡 | **关掉手机上的代理 → 让用户在主屏点一下 `S56UITests-Runner` 图标 → 再开代理**（顺序不能错；代理开着点会报「无法验证App」）。`tools/ios-runner/` 里那个放行模块**未被证实有效**，别当解法。**「VPN与设备管理」里没有「开发者App」那一节** |
