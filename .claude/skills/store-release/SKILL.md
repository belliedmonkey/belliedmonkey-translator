...
```bash
npm run issues        # 别人提的 issue / PR，我们回过没有；有没回的就 exit 1
```

发布是这个仓库最可靠的一个「一定会被走到」的路口，所以这一步放在最前面。
2026-09-09 发 1.9.0 时才偶然发现 #149 躺了 22 天没人回，而提问的人读过 `AGENTS.md`、
按规约先开了 docs-only 的领域设计 PR（#150）等评审。**发版说明里若要写「社区反馈」，
先把欠的回话还上。**
