...
let XPI = path.join(ROOT, 'belliedmonkeytranslator-firefox.xpi');   // 可被 --xpi 覆盖（见下方 --worktree）
...
(async () => {
  ...
  assertVersionIntegrity({ version: xpiVersion, what: '这个包', allowDirty: argv.includes('--allow-dirty'), tag: opt('--tag'), cwd: opt('--worktree') });
