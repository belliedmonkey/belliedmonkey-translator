...
(async () => {
  ...
  if (uuid) {
    ...
    if (!extCtx) {
      ...
      try {
        await send('browsingContext.navigate', { context: c0, url: PAGE, wait: 'complete' });
  ...
  await send('browsingContext.navigate', { context: c1, url: PAGE, wait: 'complete' }); await sleep(2500);
