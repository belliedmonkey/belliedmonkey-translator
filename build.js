...
function generateStt(dir, flavor) {
  ...
  const engines = CONFIG.filter((e) => e.flavors.includes(flavor)).map((e) => ({
    ...
    // §2.4: stored addresses for the live socket / large-file upload; null when the
    // engine has no streaming tier (the offer then only has the file tier).
    liveEndpoint: e.liveEndpoint || null, liveType: e.liveType || null,
    liveModel: e.liveModel || null, liveRate: e.liveRate || null, liveKeyProtocol: e.liveKeyProtocol || null,
    liveParams: e.liveParams || null,
    uploadEndpoint: e.uploadEndpoint || null,
  }));
