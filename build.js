...
function generateProviders(dir, flavor) {
  ...
  // The frozen pre-2026-08 endpoint snapshot rides along in this same file rather than
  // getting a load-list entry of its own: providers.gen.js is already loaded by every
  // surface that resolves an endpoint (both content_scripts blocks, options, popup,
  // review, and the app bundle), so this costs nothing to reach.
  //
  // FILTERED BY FLAVOR, like everything else here. Four of its keys are engine ids that
  // are themselves brand words (global-only engines); shipping them to China would trip
  // build.js's own compliance gate, and correctly so — a China install can never have
  // one of those as its stored engine anyway.
  const LEGACY = require('./build/legacy-endpoints.config.js');
  const shipped = new Set(providers.map((p) => p.id));
  for (const [file, cap] of [['tts.config.js', 'tts'], ['stt.config.js', 'stt']]) {
    for (const e of require('./build/' + file)) if (e.flavors.includes(flavor)) shipped.add(e.id);
  }
  const legacyPaths = {};
  for (const cap of Object.keys(LEGACY.paths)) {
    legacyPaths[cap] = {};
    for (const [id, path] of Object.entries(LEGACY.paths[cap])) {
      if (shipped.has(id)) legacyPaths[cap][id] = path;
    }
  }
  ...
  const body = `window.MT_FLAVOR = ${JSON.stringify(flavor)};\nwindow.MT_PROVIDERS = ${JSON.stringify(providers)};\n`
    + `window.MT_LEGACY_ENDPOINTS = ${JSON.stringify({ paths: legacyPaths, stripsSlash: LEGACY.stripsSlash, keyPairs: LEGACY.keyPairs })};\n`;
