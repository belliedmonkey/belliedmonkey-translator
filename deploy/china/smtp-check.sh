#!/bin/bash
# deploy/china/smtp-check.sh — 每小时一次：这台境内机器还连得上 Gmail 的发信服务器吗？
#
# 2026-09-22 用户裁定发信复用 Gmail。当天实测 587/465 都通，但境内访问 Gmail 时好时坏 ——
...
if timeout 15 openssl s_client -starttls smtp -connect smtp.gmail.com:587 -brief </dev/null 2>&1 | grep -q "CONNECTION ESTABLISHED"; then
