...
- **快速翻译（Mac App）** —— 在任何 App 里选中文字按快捷键，或用右键「服务」；也可以直接输入；选不了、复制不了的字（图片、视频画面、扫描件）就在屏幕上框一块。译文出现在鼠标旁的小面板里，不抢你正在用的 App 的焦点。屏幕上的字在你的 Mac 上识别。快捷键可改；它常驻菜单栏，可以关
- **系统翻译（iPhone / iPad App）** —— 在「设置 › App › 翻译 › 默认翻译App」里选上大肚猴翻译，之后在任何 App 里选中文字点「翻译」，弹出来的就是我们的弹层，用你自己配的引擎翻。需要 iOS 或 iPadOS 18.4 及以上
...
- **快速翻译（可选，仅 Mac App）。** 只在你主动发起时工作：在 Mac 上按快捷键、用右键「服务」、输入、或框选屏幕。你交来的文字直接发给**你自己配置的翻译引擎**，不经过我们的服务器（用免费额度时经我们的中转发给服务商，我们不保存内容）。截图里的文字在你的 Mac 上识别，截图不保存、不离开设备 —— 除非没认出来、而你点了「用我的识图引擎再试」。我们不读取你没有交给我们的内容：不监听键盘，不常驻读取剪贴板；「增强取词」默认关闭，打开后也只在你按下快捷键的那一刻替你按一次 ⌘C，读到选中的文字后把剪贴板恢复原样；剪贴板里被标记为隐藏的内容（例如密码）不读取、不发送。只有「存入复习库」开着时，原文与译文才会留在你的复习里。
- **系统翻译（可选，仅 iPhone / iPad App）。** 只在你主动发起时工作：在任何 App 里选中文字、点系统菜单里的「翻译」。选中的那段文字直接发给**你自己配置的翻译引擎**，不经过我们的服务器（用免费额度时经我们的中转发给服务商，我们不保存内容）。我们不读取你没有交给我们的内容：只有你选中的那段文字会过来，而且只在你要它的那一刻。只有「存入复习库」开着时，原文与译文才会留在你的复习里。
