...
describe('§8.10 卡面 —— 哪个状态说哪句话', () => {
  ...
  test('未登录那张卡：主按钮是登录；不再提「实时听译不含在内」（2026-09-17 起实时转写走本机，不花额度）', () => {
    const c = G.cardFor('none', { t });
    eq(c.action.id, 'signin');
    ok(!/实时/.test(c.note || ''), '那句「实时听译不在额度范围内」没有对象了 —— 本机识别不经中继，卡上再说它只会让人以为要另配什么');
    ok(!(c.links || []).some((l) => l.id === 'live'), '「另配实时引擎 →」链接已随一键卡实时格一起删除');
  });
