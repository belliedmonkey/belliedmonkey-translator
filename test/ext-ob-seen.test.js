...
//
// 这道门禁防的是未来，不是现在 —— 今天的用法都是对的。它要挡的是那种一眼看去
// 很顺的误用：「看过引导了，那应该配好了吧」。popup 里已经有一段注释在防同一族的
// 另一个误用（故意不复用 App 的 onboardSeen 键名，免得有人写出「App 看过了扩展就不用
// 看」的假联动）；一句提醒不是门禁，所以这里替人记着。
//
// 扫描面（PR3 起含 src/）：React 迁移把页面源码搬进 src/pages/，读者跟着搬家 ——
// 门禁扫的是「谁在读这个键」，不是「文件在哪个目录」。file 一律报仓库根相对路径。
...
// 扫全部出货的扩展代码 + 全部 React 页面源码（不含 dist，不含 App —— App 用的是
// 自己的 onboardSeen）。
function shipped() {
  const out = [];
  (function walk(d, exts) {
    for (const e of fs.readdirSync(d, { withFileTypes: true })) {
      ...
      if (e.isDirectory()) { if (e.name !== '_locales') walk(p, exts); }
      else if (exts.test(e.name) && !e.name.endsWith('.gen.js')
        && e.name !== 'i18n-messages.js') out.push(p);
    }
  })(path.join(ROOT, 'extension'), /\.(js|html)$/);
  (function walkSrc(d) {
    for (const e of fs.readdirSync(d, { withFileTypes: true })) {
      const p = path.join(d, e.name);
...
function hits() {
  ...
  for (const f of shipped()) {
    ...
    // src/store/schema.js 是键的**登记处**：extObSeen 出现在它的 surfaces 表里是
    // 「登记这个键」，不是「读这个键当信号」—— 不计，否则读者断言永远红。
    if (rel === path.join('src', 'store', 'schema.js')) continue;
    const src = fs.readFileSync(f, 'utf8');
    for (const [i, line] of src.split('\n').entries()) {
      if (!line.includes(KEY)) continue;
      const t = line.trim();
      if (t.startsWith('//') || t.startsWith('*')) continue;      // 注释不算用法
      found.push({ file: rel, line: i + 1, text: t });
    }
  }
  return found;
}
