# 1.15.0 发布说明（**GitHub Release / 更新日志用这一份**）
...
```
· App（iPhone / iPad / Mac）· 新：第一次打开的引导更短了 —— 登录提到第 2 屏，接着当场翻一句、听一句，最后才去打开浏览器扩展。
· App（iPhone / iPad / Mac）· 变更：同步改存境内服务器 —— 登录后，账号与学习卡片存放在位于北京的服务器上，不再出境。升级后需要重新登录一次；登录后，这台设备上的卡片会从手机直接上传到境内服务器，复习进度照旧。
· App（iPhone / iPad / Mac）· 新：「以后再设置」只关这一次 —— 下次打开，首页有一张「继续设置」卡，从上次停下的地方接着来。
· App（iPhone / iPad / Mac）· 新：点过「我已打开」但扩展其实还没开？到「设置 › 功能 › Safari 扩展」把首页那张提示找回来。
· App 与浏览器扩展 · 改进：接口地址只填了主机名、或没填 Key，离开输入框或点「测试」时就直接指出，不再先发请求再报一个看不懂的错。
· 浏览器扩展 · 改进：首次设置从 4 屏精简到 3 屏 —— 去掉单独的「采集」那一屏（采集默认就是开着的）。
```
