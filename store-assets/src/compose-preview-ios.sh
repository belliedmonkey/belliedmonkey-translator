#!/bin/bash
# iPhone App Preview（IPHONE_67 · 886×1920）合成：题卡 → App 里点「开始」→ Safari 测试页 + 画中画字幕窗滚动 → 结尾卡。
...
CARD=2.5; FADE=0.25
TOTAL=$(echo "$CARD*2 + $APP_DUR + $PIP_DUR" | bc)
...
echo "${L}：总长 $TOTAL s · 画中画段对白取外放音轨 $PA s 起"
...
[3:v]$V,fade=t=in:st=0:d=$FADE,fade=t=out:st=$(echo "$CARD-$FADE" | bc):d=$FADE[v3];\
[v0][v1][v2][v3]concat=n=4:v=1:a=0[v];\
anullsrc=r=48000:cl=stereo,atrim=duration=$(echo "$CARD + $APP_DUR" | bc)[s0];\
[4:a]aformat=sample_rates=48000:channel_layouts=stereo,loudnorm=I=-18:TP=-2,atrim=duration=$PIP_DUR,asetpts=PTS-STARTPTS,afade=t=in:st=0:d=0.3,afade=t=out:st=$(echo "$PIP_DUR-0.4" | bc):d=0.4[s1];\
anullsrc=r=48000:cl=stereo,atrim=duration=$CARD[s2];\
[s0][s1][s2]concat=n=3:v=0:a=1[dia];\
[5:a]aformat=sample_rates=48000:channel_layouts=stereo,atrim=duration=$TOTAL,afade=t=out:st=$(echo "$TOTAL-2" | bc):d=2[mus];\
