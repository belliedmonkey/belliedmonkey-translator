# 1.7.5 发布说明 · macOS（国际版 11 份 + 中国版 1 份）
...
> 「以前会怎样、现在会怎样」。
>
> 与 iOS 那份只差最后一条：界面在 Mac 上不再是放大的手机版式。那一条在 iPhone 上
> 没有意义，所以 iOS 那份里没有它 —— 写进去就是把一个不存在的东西说成新功能。
...
· 在 Mac 上，設定與引導介面不再是被放大的手機版式：三步說明並排顯示，按鈕
  也是正常的按鈕大小。
```
...
· On the Mac, the settings and setup screens are no longer a blown-up phone layout:
  the three steps sit side by side and the buttons are button-sized again.
```
...
· Mac では、設定と初回案内が「引き伸ばしたスマホ画面」ではなくなりました。3つの手順が
  横に並び、ボタンもボタンらしい大きさになります。
```
...
· 팟캐스트 모드: 언어를 알 수 없는 카드도 읽어 줍니다. iPhone에서는 지금까지 이런
  카드가 한 장도 재생되지 않았습니다.
· Mac에서는 설정과 시작 안내가 더 이상 확대된 휴대폰 화면이 아닙니다. 세 단계가
  나란히 놓이고 버튼도 버튼다운 크기가 됩니다.
```
...
· Sur Mac, les écrans de réglages et de configuration ne sont plus une mise en page de
  téléphone agrandie : les trois étapes sont côte à côte et les boutons ont une taille normale.
```
...
· Auf dem Mac sind Einstellungen und Einrichtung keine vergrößerte Handy-Ansicht mehr:
  Die drei Schritte stehen nebeneinander und Buttons haben wieder Button-Größe.
```
...
· En el Mac, los ajustes y la configuración inicial ya no son un diseño de teléfono
  ampliado: los tres pasos van uno al lado del otro y los botones tienen tamaño de botón.
```
...
· No Mac, as telas de ajustes e de configuração deixaram de ser um layout de celular
  ampliado: os três passos ficam lado a lado e os botões têm tamanho de botão.
```
...
· На Mac настройки и первое знакомство больше не выглядят как растянутый экран
  телефона: три шага стоят рядом, а кнопки снова нормального размера.
```
...
· وضع البودكاست: البطاقات التي تعذّر تحديد لغتها تُقرأ الآن. على iPhone لم تكن أيٌّ
  منها تُشغَّل من قبل.
· على الـ Mac، لم تعد شاشات الإعدادات والتعريف تخطيطاً مكبَّراً لهاتف: الخطوات الثلاث
  تظهر جنباً إلى جنب، وأحجام الأزرار طبيعية.
```
