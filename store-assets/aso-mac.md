> **生成的文件，不要手改。** 由 `node scripts/gen-aso-mac.js` 从 `aso.md` + `aso-mac-section.json` 生成：
...
【关于账号与数据】
翻译本身不需要账号：API Key 只保存在本机，翻译请求由你的设备直接发往你选择的服务商，不经过我们的服务器。实时字幕与设备内置转写都不录音；设备内置朗读（离线模型）首次使用时会下载一次模型文件（下载地址由我们的服务器给出，不含任何个人数据；模型目前托管在魔搭 ModelScope 的公开仓库）。App 内提供可选登录，仅用于在你自己的设备之间同步学习进度。无广告、无第三方统计、不收集你的浏览记录。
